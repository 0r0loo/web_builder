...
export function useDragAndDrop() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
      ...
      if (currentPage) {
        ...
        if (targetNode) {
          ...
          if (targetMetadata && !targetMetadata.allowChildren) {
            ...
            console.warn(
              `Cannot drop into ${targetNode.type}: children not allowed`,
            );
            setActiveId(null);
            setOverId(null);
            return;
    ...
    // 캔버스 내 컴포넌트 이동/순서 변경
    else if (activeId !== overId && dragData.type === "canvas-node") {
      const currentPage = getCurrentPage();
      if (!currentPage) {
        setActiveId(null);
        setOverId(null);
      ...
      // sortable data에 isContainer가 true이거나, droppable data에 type이 canvas-container인 경우
      const isOverContainer =
        overData?.type === "canvas-container" || overData?.isContainer === true;
      ...
      if (isOverContainer) {
        ...
        moveNode(dragData.nodeId, overId);
      } else {
        // 순서 변경 (형제 간)
