...
export function ComponentRenderer({
  ...
  // 컨테이너면 droppable ref 설정, sortable은 선택되었을 때만
  const setNodeRef = (element: HTMLElement | null) => {
    if (isContainer) {
      setDroppableRef(element);
    }
    // sortable은 항상 설정 (disabled 옵션으로 제어)
  ...
  return (
    <ContextMenu.Root>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content className="min-w-[200px] rounded-lg border border-zinc-200 bg-white p-1 shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
          <ContextMenu.Item
